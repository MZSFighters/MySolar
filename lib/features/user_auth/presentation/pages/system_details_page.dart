...
class systemDetailsIInformation extends StatefulWidget {
  ...
  @override
  State<systemDetailsIInformation> createState() =>
      _systemDetailsIInformationPage();
}
...
class _systemDetailsIInformationPage extends State<systemDetailsIInformation> {
  ...
  TextEditingController _lowestBatteryPercentageController =
      TextEditingController();
  TextEditingController _maxInverterCapacityController =
      TextEditingController();
  ...
  void initState() {
    ...
    DatabaseReference dbRef =
        FirebaseDatabase.instance.ref("systemDetailsInformation/$USER");
    dbRef.onValue.listen((event) {
      if (event.snapshot.exists) {
        setState(() {
          ...
          _lowestBatteryPercentageController.text =
              powerInfo["lowestBatteryPercentage"];
          _maxInverterCapacityController.text =
              powerInfo["maxInverterCapacity"];
        });
      } else {
        print("no data");
      }
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 30),
                  _buildInputRow("[1] Solar Panels Max Output (kW/hr) :",
                      _maxPowerOutputController),
                  _buildInputRow("[2] Maximum Battery Storage (kW) :",
                      _batteryCapacityController),
                  _buildInputRow("[3] Lowest Acceptable Battery Percentage % :",
                      _lowestBatteryPercentageController,
                      isPercentage: true),
                  _buildInputRow("[4] Maximum Inverter Power Threshold (kW)",
                      _maxInverterCapacityController),
                  SizedBox(height: 30),
                  GestureDetector(
                    onTap: _power,
                    child: Container(
  ...
  void _power() async {
    if (_formKey.currentState!.validate()) {
      ...
      double lowestBatteryPercentage =
          double.parse(_lowestBatteryPercentageController.text);
      double maxInverterCapacity =
          double.parse(_maxInverterCapacityController.text);
