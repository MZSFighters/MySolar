...
class GraphInitiator extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<SystemDetails?>(
      ...
      builder: (context, snapshotSystemDetails) {
        ...
        } else if (snapshotSystemDetails.hasError || !snapshotSystemDetails.hasData) {
          print('${snapshotSystemDetails.error}');
          return Scaffold(body: Center(child: Text('Error : ${snapshotSystemDetails.error}')));
        }

        final SystemDetails systemDetails = snapshotSystemDetails.data!;
        final double maxOutput = systemDetails.maxPowerOutput;
        final double possibleStorage = systemDetails.batteryCapacity * (1 - systemDetails.lowestBatteryPercentage / 100);
            ...
            return FutureBuilder<List<List<String>>>(
              ...
              builder: (context, snapshotAppliances) {
                if (snapshotAppliances.connectionState == ConnectionState.waiting) {
                  ...
                  return Scaffold(body: Center(child: Text('Error: ${snapshotAppliances.error}')));
                }
  ...
  static Future<List<Map<String, dynamic>>> finalOutputData(SystemDetails systemDetails) async {
