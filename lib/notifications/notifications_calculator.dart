...
class NotificationsCalculator {
  ...
  Future<SystemDetails?> _fetchSystemDetails() async {
    DatabaseReference dbRef = FirebaseDatabase.instance.ref("systemDetailsInformation/$userId");
  ...
  Future<List<String>> calculateNotifications() async {
    ...
    try {
      ...
      List<double> consumptionPerMinute = await DeviceConsumption(userId: userId, firestore: firestore).calculateMinuteConsumption(apiHour);
      ...
      for (int i = 0; i < consumptionPerMinute.length; i++) {
        if (consumptionPerMinute[i] > systemDetails.maxInverterCapacity) {
          notifications.add("Warning! : Consumption at time $apiHour : ${i%60} exceeds your max inverter capacity (${systemDetails.maxInverterCapacity} kW)");
        }
      }
      ...
      List<double> finalOutputKw = getFinalOutput(finalOutput);
        ...
        for (int i = 0; i < 720; i++) { //for each minute
          
          for (int a = 0; a < adjusted.length; a++) { // for each period of loadshedding 
              ...
              int endMinutes = (end + (apiHour*60))%60;
                  for(int s = start; s < end; s++){ //loop through the minutes of that loadshedding
                      if(i >= start || i <= end){ //only check if minute falls between loadshedding period
                      if(finalOutputKw[i] < 0 ){
                        notifications.add('Warning! : You will be using grid during the load shedding period from $startHour:$startMinutes - $endHour:$endMinutes');
                        break;
                      }
                      }
                  }
  ...
  Future<List<Map<String, dynamic>>> _calculateFinalOutput(SystemDetails systemDetails) async {
